export const selectRestoWithPizza = (reduxState) => {
  const restaurants = reduxState.restaurants.allRestos;
  ...
  const restoWithPizza = restaurants.map((resto) => {
    ...
    const replacePizzas = resto.pizzas.map((pizzaId) => {
      return pizzas.find((fullPizza) => pizzaId === fullPizza.id);
    });
    // Return the same restaurant object but with a switched pizzas array.
